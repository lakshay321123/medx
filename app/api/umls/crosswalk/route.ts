...
export async function GET(req: NextRequest) {
  ...
  const cui = sp.get('cui') || '';
  const target = sp.get('target') || 'ICD10CM';
  ...
  const mappings = atoms.map((a: any) => ({
    code: a.code?.code, term: a.name, source: a.rootSource, tty: a.termType, ui: a.ui
  }));
