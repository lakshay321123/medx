...
export async function GET() {
  ...
  try {
    ...
    (feed.items || []).slice(0, 5).forEach(i =>
      items.push({ title: i.title, url: i.link, source: 'WHO', time: i.pubDate })
    );
  } catch {}

  // ClinicalTrials.gov (example query)
  try {
    const r = await fetch(
      'https://clinicaltrials.gov/api/v2/studies?filter.overallStatus=RECRUITING&query.term=thyroid&sort=lastUpdatePostDate:desc&pageSize=5'
    );
    if (r.ok) {
  ...
  try {
    ...
    const r = await fetch(
      `https://api.fda.gov/drug/label.json?search=effective_time:[20240101+TO+20251231]&limit=5${
        key ? `&api_key=${key}` : ''
      }`
    );
    if (r.ok) {
      ...
      (j.results || []).forEach((s: any) =>
        items.push({
          title: (s.openfda?.brand_name || ['Drug label update']).join(', '),
