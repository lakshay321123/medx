...
const ENABLED = String(process.env.DIET_TEMPLATES || "").toLowerCase() === "true";
...
export async function GET(req: NextRequest) {
  if (!ENABLED)
    return NextResponse.json({ error: "Diet templates disabled" }, { status: 404 });
  ...
  if (!Array.isArray(index.diet) || !index.diet.includes(condition))
    return NextResponse.json({ error: "not found" }, { status: 404 });

  const key = `diet:${condition}:${lang}`;
  const now = Date.now();
  ...
  if (entry && now - entry.ts < TTL) {
    ...
    try {
      const file = path.join(process.cwd(), `data/plans/diet/${condition}.${lang}.json`);
      ...
      if (lang !== DEFAULT_LANG) {
        ...
        try {
          const file = path.join(
            process.cwd(),
            `data/plans/diet/${condition}.${lang}.json`
          );
          data = await loadJSON(file);
        } catch {
          return NextResponse.json({ error: "not found" }, { status: 404 });
    ...
    cache.set(`diet:${condition}:${lang}`, { data, ts: now });
  }
