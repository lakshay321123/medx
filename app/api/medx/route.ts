...
Return STRICT JSON like: {"intent":"NEARBY","keywords":["pharmacy"]}`;
...
export async function POST(req: NextRequest){
  const { query, mode, coords } = await req.json(); // now accepts coords from client
  ...
  try {
    if (intent === 'NEARBY') {
      const kind = keywords[0] || query;
      if (!coords || typeof coords.lat !== 'number' || typeof coords.lng !== 'number') {
        sections.needsLocation = true;
        sections.kind = kind;
      } else {
        const r = await fetchWithTimeout(`${process.env.NEXT_PUBLIC_BASE_URL || ''}/api/nearby`, {
