...
// Minimal adult ranges (illustrative). Extend as needed.
...
// Tolerant matcher: "Hb 13.2 g/dL", "Glucose: 126 mg/dL", etc.
...
function mapToRanges(cands: ReturnType<typeof findCandidates>) {
  ...
  for (const c of cands) {
    ...
    if (!matchKey) { items.push({ label: c.name, key: k, value: c.value, unit: c.unit, status: 'unknown' }); continue; }
    const ref = RANGES[matchKey];
    let status: 'low'|'normal'|'high' = 'normal';
    if (c.value < ref.min) status = 'low';
    else if (c.value > ref.max) status = 'high';
    items.push({ label: ref.label, key: matchKey, value: c.value, unit: c.unit || ref.unit, ref: { min: ref.min, max: ref.max, unit: ref.unit }, status });
  }
  return items;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!text) {
      return NextResponse.json({ ok:true, text:'', values:[], summary:'No selectable text found (the PDF may be a scan).' });
    }
    ...
    return NextResponse.json({
      ...
      summary,
      disclaimer: 'Automated summary for educational use only; not medical advice. Please consult your clinician.'
    });
  } catch (e:any) {
