...
async function analyzePrescription(text: string) {
  ...
  const cleaned: string[] = words.map((w: string) => cleanToken(w)).filter(Boolean);
  ...
  try {
    ...
    if (!text) {
      return NextResponse.json({
        ...
        preview: '',
        note: 'No selectable text found (may be a scanned PDF). OCR needed.'
      });
    }
