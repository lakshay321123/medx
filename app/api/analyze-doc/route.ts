...
// Heuristics (same as your version)
function looksLikeBloodReport(text: string): boolean {
  const t = text.toLowerCase();
  const labHints = ['hemoglobin','hematocrit','hct','hb','wbc','rbc','platelet','platelets','mcv','mch','mchc','rdw','neutrophil','lymphocyte','monocyte','eosinophil','basophil','glucose','creatinine','bun','sodium','potassium','chloride','bicarbonate','calcium','alt','ast','alkaline phosphatase','bilirubin','tsh','t3','t4','ldl','hdl','triglycerides'];
  let hits = 0; for (const h of labHints) if (t.includes(h)) hits++;
...
function looksLikePrescription(text: string): boolean {
  ...
  let score = 0; if (dose.test(text)) score++; if (freq.test(text)) score++; if (rxWords.test(text.toLowerCase())) score++; if (medDose.test(text)) score++;
  return score >= 2;
}

// Rx mapping (simplified)
...
async function rxcuiForName(name: string): Promise<string | null> {
  ...
  const id = j?.idGroup?.rxnormId?.[0] as string | undefined;
  return id ?? null;
}
...
function labCandidates(text: string) {
  ...
  while ((m = VAL_RE.exec(text)) !== null) {
    const name = m[1].trim(); const value = Number(m[2]); const unit = m[3]?.trim();
...
function labMap(cs: ReturnType<typeof labCandidates>) {
  return cs.map(c => {
    ...
    const ref = RANGES[rk]; const unit = c.unit || ref.unit;
...
async function analyzeBlood(text: string) {
  ...
  return { values: items, summary, disclaimer: 'Automated summary for education only; not medical advice.' };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      // IMPORTANT: still return JSON (never empty)
    ...
    if (detectedType === 'prescription') {
      ...
      return NextResponse.json({ ok:true, detectedType, ...rx, note: rx.meds.length ? undefined : 'No clear medicines detected.' });
    }

    return NextResponse.json({ ok:true, detectedType, preview: text.replace(/\s+/g, ' ').slice(0, 1000) });
  } catch (e:any) {
