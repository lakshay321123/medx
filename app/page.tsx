...
If CONTEXT has codes or trials, explain them in plain words and add links. Avoid medical advice.`;
  ...
  async function handleXray(file: File) {
    if (!file) return;
    setBusy(true);
    try {
      ...
      const lines: string[] = [];
      lines.push(`**Imaging analysis – ${file.name}**`);
      if (Array.isArray(data.predictions)) {
        lines.push('**Predictions:**');
        data.predictions.forEach((p: any) =>
          lines.push(`- ${p.label}: ${(Number(p.score) * 100).toFixed(0)}%`)
        );
      }
      ...
      setMessages(prev => [
        ...prev,
        { role: 'assistant', content: `⚠️ Imaging upload failed: ${String(e?.message || e)}` }
      ]);
    } finally {
      setBusy(false);
    }
              ...
              <div style={{ marginTop:10, textAlign:'right', display:'flex', justifyContent:'flex-end', gap:8 }}>
                ...
                <label className="item" style={{ cursor:'pointer' }}>
                  ...
                  <input
                    ...
                    onChange={(e)=>{ const f=e.target.files?.[0]; if(f) handleXray(f); e.currentTarget.value=''; }}
                  />
                </label>
              </div>
            </div>
