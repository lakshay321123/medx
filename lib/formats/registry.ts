...
export const FORMATS: FormatMeta[] = [
  ...
  },
  {
    ...
    systemHint: [
      ...
      'Columns (exact order): Topic | Mechanism/How it works | Expected benefit | Limitations/Side effects | Notes/Evidence',
      'Populate **every** cell; if truly unknown, put a single dash "-" (do not leave empty).',
      'Keep cells concise (≤ 18–22 words). Use plain phrases, not sentences.',
      'Rows: include the main item plus 4–8 reasonable alternatives (Alt 1, Alt 2 … only if you lack real names).',
      'Mechanism = “how/why it works”; Expected benefit = primary upside; Limitations/Side effects = risks/caveats; Notes/Evidence = 1–2 factual anchors (study type, guideline, order-of-magnitude).',
      'Do **not** write “Short answer”, “N/A”, or commentary. Output the table and nothing else.',
      '',
      'Example:',
      '| Topic | Mechanism/How it works | Expected benefit | Limitations/Side effects | Notes/Evidence |',
      '|-------|-------------------------|------------------|--------------------------|----------------|',
      '| Diet Coke vs Coke | Aspartame provides sweetness without sugar | Lower calories; no sugar spike | Dental erosion; artificial sweetener concerns | ADA/WHO guidance; observational studies |',
    ].join('\n'),
  },
  {
    id: 'json_structured',
