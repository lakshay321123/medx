...
import {
  ...
  TRIGGER_TEST_FOR,
  TRIGGER_DOC_WORKUP,
  SUGGESTION_LABELS_EN,
  type SuggestionKey,
} from "@/data/suggestions";

export function getDefaultSuggestions(mode: ModeState): SuggestionKey[] {
  if (mode.base === "doctor") {
...
export function getInlineSuggestions(query: string, mode: ModeState): SuggestionKey[] {
  ...
  return pool
    .filter(key => (SUGGESTION_LABELS_EN[key] ?? key).toLowerCase().includes(q))
    .slice(0, 8);
}
