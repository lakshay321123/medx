...
export function calc_perc_rule(i: PERCInputs): { positive_criteria:number; perc_negative:boolean } {
  const criteria = [
    i.age_lt_50, i.hr_lt_100, i.spo2_ge_95, i.no_hemoptysis, i.no_estrogen_use,
    i.no_prior_dvt_pe, i.no_unilateral_leg_swelling, i.no_recent_surgery_or_trauma
  ];
  const positives = criteria.filter(Boolean).length;
  return { positive_criteria: positives, perc_negative: positives === 8 };
}
...
const def = {
  ...
  inputs: [
    ...
    { id: "spo2_ge_95", label: "SaO₂ ≥95%", type: "boolean" },
  ...
  run: (args: PERCInputs) => {
    ...
    return { id: "perc_rule", label: "PERC Rule (PE)", value: r.positive_criteria, unit: "criteria", precision: 0, notes, extra: r };
  },
};
