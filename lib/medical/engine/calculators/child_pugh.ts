...
function scoreBilirubin(b: number): number {
  if (b < 2) return 1;
  if (b <= 3) return 2;
  return 3;
}
function scoreAlbumin(a: number): number {
  if (a > 3.5) return 1;
  if (a >= 2.8) return 2;
  return 3;
}
function scoreINR(i: number): number {
  if (i < 1.7) return 1;
  if (i <= 2.3) return 2;
  return 3;
}
function scoreAscites(a: "none" | "mild" | "moderate"): number {
  if (a === "none") return 1;
  if (a === "mild") return 2;
  return 3;
}
function scoreEncephalopathy(e: "none" | "mild" | "moderate"): number {
  if (e === "none") return 1;
  if (e === "mild") return 2;
  return 3;
}
...
export function calc_child_pugh({
  ...
  const s =
    scoreBilirubin(bilirubin_mg_dl) +
    scoreAlbumin(albumin_g_dl) +
    scoreINR(inr) +
