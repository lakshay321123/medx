...
export function gradeCRS(params: {
  ...
  if (temp_C == null) return { grade: null as number | null, notes: ["Need temperature."] };
...
export function gradeICANS(params: {
  ...
  if (ice_score_0_10 == null) return { grade: null as number | null, notes: ["Need ICE score."] };
...
// Wrap the pure functions so run() returns a CalcResult shape (must include id & label)
register({
  id: "astct_crs",
