...
export async function finalizeCalc(spec: CalcSpec): Promise<CalcVerdict> {
  try {
    const precision = spec.precision ?? 2;
    const tolerancePct = spec.tolerancePct ?? 0;
    const timeoutMs = spec.timeoutMs ?? 7000;
    let attempts = 0;

    if (!Number.isFinite(spec.localResult)) {
      return { status: spec.strict ? "blocked" : "ok", final: 0, tier: "local", attempts, agreeWithLocal: false, deltaAbs: NaN, deltaPct: NaN, reason: "Local non-finite." };
    }
    if (process.env.CALC_AI_DISABLE === "1") {
      return { status: "ok", final: Number(spec.localResult.toFixed(precision)), tier: "local", attempts, agreeWithLocal: true, deltaAbs: 0, deltaPct: 0, explanation: "AI verification disabled." };
    }
    ...
    if (hasOpenAI()) {
      for (const verbose of [false, true]) {
        ...
        if (n !== null) {
          ...
          if (pass) return { status: "ok", final, tier: "openai", attempts, agreeWithLocal: true, deltaAbs, deltaPct, explanation: (r as any).explanation || "" };
          if (spec.strict) return { status: "blocked", final: Number(spec.localResult.toFixed(precision)), tier: "local", attempts, agreeWithLocal: false, deltaAbs, deltaPct, explanation: (r as any).explanation || "", reason: "OpenAI outside tolerance (strict)." };
    ...
    if (hasGroq()) {
      for (const verbose of [false, true]) {
        ...
        if (n !== null) {
          const { final, deltaAbs, deltaPct, pass } = evalVsLocal(n, spec.localResult, precision, tolerancePct);
          if (spec.strict) return { status: "blocked", final: Number(spec.localResult.toFixed(precision)), tier: "local", attempts, agreeWithLocal: pass, deltaAbs, deltaPct, explanation: (r as any).explanation || "", reason: "Strict mode requires OpenAI." };
          if (pass) return { status: "ok", final, tier: "groq", attempts, agreeWithLocal: true, deltaAbs, deltaPct, explanation: (r as any).explanation || "" };
    ...
    return { status: spec.strict ? "blocked" : "ok", final: Number(spec.localResult.toFixed(precision)), tier: "local", attempts, agreeWithLocal: true, deltaAbs: 0, deltaPct: 0, explanation: "AI unavailable; using local.", reason: spec.strict ? "Strict mode requires OpenAI." : undefined };
  } catch (err) {
    console.error("[triage] Fatal:", err);
    const precision = spec.precision ?? 2;
    return { status: "ok", final: Number(spec.localResult.toFixed(precision)), tier: "local", attempts: 0, agreeWithLocal: true, deltaAbs: 0, deltaPct: 0, reason: "Caught fatal error; returned local." };
  }
}
