...
export type DetectedEntities = {
  ...
  dates?: string[];              // every date-like token we detect
  compareWindow?: { a: string; b: string } | null; // first two date tokens
};
...
function normalize(s: string): string {
  return s
    .toLowerCase()
    .normalize("NFKD")
    .replace(/[^\x00-\x7F]/g, "")    // strip diacritics
    .replace(/[^a-z0-9\s:/\-]/g, " ") // keep date separators
    .replace(/\s+/g, " ")
    .trim();
}
...
// Quick reverse lookup for fast detection
...
// Matches:
//  - 2025-10-01
//  - 01/10/2025 or 1/10/2025
//  - Oct 2025, October 2025, 10/2025
//  - 2025/10/01 (less common but allowed by the regex)
const DATE_REGEX =
...
function extractDates(q: string): string[] {
  ...
  // Normalize months to Title Case short form where possible
...
function extractMetric(q: string): { metric?: string; metricAlias?: string } {
  ...
  // Try plain token equals (e.g., "ldl?")
  for (const [canonical, aliases] of Object.entries(METRIC_ALIASES)) {
...
export function detectIntentAndEntities(query: string): DetectResult {
  ...
  // 2) Token overlap scoring (fallback / tie-breaker)
  ...
  // If it mentions x-ray/mri/ct/usg/echo → interpret_report bias
