// lib/intents.ts
// MedX — Exhaustive Intents Library (Single-File Edition)
// ------------------------------------------------------------------
// What this provides
...
// - Intent-to-outline mapping → returns concise markdown structure blocks
...
export function detectFollowupIntent(text: string): FollowupIntent {
  ...
  if (/\b(latest|newest|recent).*\b(trial|trials|study|studies|research)\b|\bclinical trial(s)?\b/.test(q)) return "trials";
  return null;
}
...
/** Globbing to RegExp: '*' → '\\w*', spaces flexible, hyphen optional */
...
const PATIENT_SPECS: IntentSpec[] = [
  ...
  { slug: "severity_serious", title: "Serious / red flags", triggers: ["is it serious", "should i be worried", "red flags", "danger signs", "emergency"], outline: ["## **Red flags — seek care now**", "## **What is okay to watch at home**"] },
