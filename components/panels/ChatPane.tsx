    ...
    if (isProfileThread && proactive && !pendingFile && note.trim()) {
      ...
      try {
        if (proactive.kind === 'predispositions') {
          ...
          for (const m of meds) {
            await fetch('/api/observations', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ kind:'medication', value_text:m, meta:{ source:'chat' }, thread_id:'med-profile', observed_at:new Date().toISOString() }) });
          }
          ack(`Noted — recorded medications: ${meds.join('; ')}`);
        } else {
          const kg = parseFloat(text.replace(/[^\d.]/g,''));
          if (Number.isFinite(kg)) {
            await fetch('/api/observations', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ kind:'weight', value_text:`${kg} kg`, value_num:kg, unit:'kg', thread_id:'med-profile', observed_at:new Date().toISOString() }) });
        ...
        if (v?.ok && v?.suggestion && window.confirm(`Did you mean "${v.suggestion}"?`)) {
          await safeJson(fetch('/api/observations', {
            ...
            body: JSON.stringify({
              ...
              unit: null,
              meta: { source: 'chat', confirmed: true },
