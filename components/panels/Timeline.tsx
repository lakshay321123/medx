...
export default function Timeline(){
  ...
  useEffect(() => {
    ...
    const sourceSignature = JSON.stringify([
      meta.summary_long ?? null,
      meta.summary ?? null,
      meta.text ?? null,
      active?.value_text ?? null,
    ]);
    const cacheKey = `${active.id}:${lang}:${sourceSignature}`;
    ...
    let cancelled = false;
    fetch(`/api/timeline/summary?lang=${encodeURIComponent(lang)}`, {
      method: "POST",
      ...
      .then(payload => {
        ...
        const translatedData: DrawerTranslation = {
          summaryLong:
            payload?.summaryLongLocalized ?? payload?.summaryLocalized ?? payload?.summaryShortLocalized ?? null,
          summaryShort:
            payload?.summaryShortLocalized ?? payload?.summaryLocalized ?? payload?.summaryLongLocalized ?? null,
          text: payload?.textLocalized ?? payload?.fullTextLocalized ?? null,
          valueText: payload?.valueTextLocalized ?? null,
        };
