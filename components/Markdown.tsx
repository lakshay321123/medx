...
export default function Markdown({ text }: { text: string }) {
  ...
  const withSafeLinks = sanitized.replace(/<a\s+[^>]*href="([^"]+)"[^>]*>(.*?)<\/a>/gi, (_m, href, inner) => {
    ...
    if (!safe) {
      return `<span class="inline-flex items-center rounded-full border border-so-border dark:border-gray-700 px-2 py-1 text-xs text-so-muted" title="Link unavailable">${inner}</span>`;
    }
    ...
    class="inline-flex items-center gap-1 rounded-full border border-so-border dark:border-gray-700 px-2 py-1 text-xs font-medium text-so-text dark:text-so-text shadow-sm hover:bg-so-bg dark:hover:bg-gray-800 transition">
