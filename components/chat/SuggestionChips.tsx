      ...
      {translated.map((s) => {
              ...
              <div className="flex flex-wrap gap-1.5">
                {options.map((opt, i) => (
                  <button
                    key={i}
                    type="button"
                    className="rounded-full border border-[var(--so-accent,#06B6D4)] px-3.5 py-1.5 text-[13px] font-medium text-[var(--so-accent,#06B6D4)] transition hover:bg-[var(--so-accent,#06B6D4)] hover:text-white dark:border-[#06B6D4] dark:text-[#06B6D4] dark:hover:bg-[#06B6D4] dark:hover:text-black"
        ...
        return (
          <button
            key={s.id}
            type="button"
            className="block w-full text-left rounded-xl border border-[var(--so-border,#E5E5EA)] px-3.5 py-2.5 text-[13px] text-[var(--so-text,#000)] transition hover:border-[var(--so-accent,#06B6D4)] hover:bg-[rgba(6,182,212,0.04)] dark:border-[var(--so-border,#2C2C2E)] dark:text-[var(--so-text,#fff)] dark:hover:border-[#06B6D4]"
            onClick={() => onAction(s)}
          >
            {s.label}
          </button>
