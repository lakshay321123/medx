  ...
  return (
    <button type="button" aria-pressed={on} onClick={toggle}
      className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm transition
        ...
        : "bg-so-bg text-so-text border-so-border dark:bg-so-card dark:text-gray-100 dark:border-gray-700"}`}>
      <span className={`inline-block h-2.5 w-2.5 rounded-full ${on?"bg-emerald-500":"bg-so-muted"}`} />
